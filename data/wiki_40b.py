...
def process_tf_dataset(ds, num_examples, output_file):
    # Turn to a numpy df so that we can easily extract text
    numpy_items = tfds.as_numpy(ds)

    with open(output_file, "a") as f:
        for _, item in zip(range(num_examples), numpy_items):
...
def process_lang(lang_code, args):
    ...
    process_tf_dataset(
        ds_train, args.num_train_examples, args.output_prefix + lang_code + ".train"
    )

    ds_test = tfds.load(f"wiki40b/{lang_code}", split="test", shuffle_files=True)
    process_tf_dataset(
        ds_test, args.num_test_examples, args.output_prefix + lang_code + ".test"
    )

    ds_valid = tfds.load(f"wiki40b/{lang_code}", split="validation", shuffle_files=True)
    process_tf_dataset(
        ds_valid, args.num_valid_examples, args.output_prefix + lang_code + ".valid"
    )
